  ...
  <div class="min-h-screen bg-slate-50 flex font-sans antialiased text-slate-900">
    ...
    <aside class="w-64 bg-slate-900 text-slate-300 flex-shrink-0 hidden md:flex flex-col border-r border-slate-800">
      <div class="h-16 flex items-center px-6 border-b border-slate-800">
        <span class="text-white font-black tracking-tighter text-xl">WIP<span class="text-indigo-500">LITE</span></span>
      </div>
      
      <nav class="flex-1 p-4 space-y-1">
        <p class="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-4 px-3">Menu Principal</p>
        ...
        <Link href="/admin/dashboard" 
          ...
          <i class="pi pi-home mr-3 text-lg" :class="isActive('/admin/dashboard') ? 'text-indigo-400' : 'text-slate-500 group-hover:text-indigo-400'"></i>
        ...
        <Link href="/admin/employees" 
              ...
              class="flex items-center p-3 rounded-xl transition-all duration-200 group">
          <i class="pi pi-users mr-3 text-lg" :class="isActive('/admin/employees') ? 'text-indigo-400' : 'text-slate-500 group-hover:text-indigo-400'"></i>
        ...
        <Link href="/admin/campaigns" 
              ...
              class="flex items-center p-3 rounded-xl transition-all duration-200 group">
          <i class="pi pi-flag mr-3 text-lg" :class="isActive('/admin/campaigns') ? 'text-indigo-400' : 'text-slate-500 group-hover:text-indigo-400'"></i>
      ...
      class="flex items-center p-3 rounded-xl transition-all duration-200 group">
  <i class="pi pi-table mr-3 text-lg" :class="isActive('/planning-models') ? 'text-indigo-400' : 'text-slate-500 group-hover:text-indigo-400'"></i>
...
<Link href="/planning-assignments" 
      ...
      class="flex items-center p-3 rounded-xl transition-all duration-200 group">
  <i class="pi pi-users mr-3 text-lg" :class="isActive('/planning-assignments') ? 'text-indigo-400' : 'text-slate-500 group-hover:text-indigo-400'"></i>
  <span class="font-medium">Affectations</span>
      ...
      <div class="p-4 border-t border-slate-800">
         <div class="bg-slate-800/50 p-3 rounded-xl flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center text-white font-bold text-xs">AD</div>
            <div class="overflow-hidden">
               <p class="text-xs font-bold text-white truncate">Administrateur</p>
               <p class="text-[10px] text-slate-500 truncate">admin@wiplite.com</p>
            </div>
         </div>
      </div>
    </aside>
    ...
    <div class="flex-1 flex flex-col">
      ...
      <header class="h-16 bg-white/80 backdrop-blur-md sticky top-0 z-10 flex items-center justify-between px-8 border-b border-slate-200">
        ...
        <div class="flex items-center gap-3">
          ...
          <Button 
            ...
            v-badge.danger="$page.props.auth.notifications.length || null" 
            text 
            plain
          ...
          <button class="flex items-center gap-2 hover:bg-slate-50 p-1 rounded-full transition-all">
      ...
      <!-- OverlayPanel épuré -->
      <OverlayPanel ref="op" style="width: 380px" class="shadow-2xl border-slate-200 rounded-2xl overflow-hidden">
        <div class="flex flex-col">
          <div class="p-4 border-b border-slate-100 flex justify-between items-center">
             <span class="font-bold text-slate-800">Notifications</span>
             <span v-if="$page.props.auth.notifications.length" class="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded-md font-bold uppercase">Nouveau</span>
          </div>
          ...
          <div class="max-h-[400px] overflow-y-auto">
            <div v-if="$page.props.auth.notifications.length > 0">
              <div v-for="notif in $page.props.auth.notifications" 
                ...
                <div class="w-10 h-10 rounded-full bg-blue-50 flex-shrink-0 flex items-center justify-center">
                   <i class="pi pi-calendar text-blue-500"></i>
                </div>
                <div class="flex-1">
                  <p class="text-sm text-slate-700 leading-snug mb-1">{{ notif.data.message }}</p>
                  <p class="text-[11px] text-slate-400 flex items-center italic">
                     <i class="pi pi-clock mr-1 text-[10px]"></i> {{ new Date(notif.created_at).toLocaleDateString() }}
                  </p>
                </div>
              </div>
            </div>
            <div v-else class="p-12 text-center flex flex-col items-center">
              <i class="pi pi-bell-slash text-slate-200 text-4xl mb-3"></i>
              <p class="text-slate-400 text-sm">Tout est à jour !</p>
            </div>
          </div>

          <div v-if="$page.props.auth.notifications.length > 0" class="p-3 bg-slate-50 text-center">
            <Link href="/notifications" class="text-xs text-indigo-600 font-bold hover:text-indigo-800">
